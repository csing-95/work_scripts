...
# =========================================================
# Post-OCR: remove a known suffix from flat outputs (e.g. "_OCR")
# =========================================================

def strip_suffix_in_flat_outputs(
    flat_folder: str,
    output_ext: str = ".pdf",
    suffix_to_remove: str = "_OCR",
    report_path: str = "",
    log_fn=print,
):
    """
    Renames files in a flat folder:
      base + suffix + ext  ->  base + ext

    Example:
      page-0001__000001_OCR.pdf -> page-0001__000001.pdf

    Collision handling:
      if target exists, appends _1, _2, ...
    """
    ...
    suffix_to_remove = (suffix_to_remove or "").strip()
    ...
    for name in os.listdir(flat_folder):
        ...
        low = name.lower()
        if not low.endswith(output_ext):
            continue

        if suffix_to_remove:
            # must end with suffix + ext (case-insensitive)
            if not low.endswith((suffix_to_remove.lower() + output_ext)):
                skipped += 1
                continue
            new_name = name[: -len(suffix_to_remove) - len(output_ext)] + output_ext
        else:
            # nothing to remove
            skipped += 1
            continue

        dest_full = os.path.join(flat_folder, new_name)
        ...
        try:
            ...
            log_fn(f"✅ Renamed: {name} -> {os.path.basename(final_dest)}")
            ...
            log_fn(f"❌ ERROR: {name} -> {e}")

    summary = {
        "flat_folder": flat_folder,
        "output_ext": output_ext,
        "suffix_removed": suffix_to_remove,
        "renamed": renamed,
        "skipped": skipped,
...
def recreate_from_original_tree(
    ...
    output_ext: str = ".pdf",
    output_suffix: str = "",
    # After placing, optionally strip token like __000001 from filenames
    strip_token_after_place: bool = False,
    token_separator: str = "__",
    token_regex: str = r"\d+",
    report_path: str = "",
    log_fn=print,
):
    ...
    for root, _, files in os.walk(original_root):
        ...
        for file in files:
            ...
            # Optional: after placing, strip token like __000001 from final filename
            def _maybe_strip_token(path_in: str) -> str:
                if not strip_token_after_place:
                    return path_in
                sep = (token_separator or "__")
                pat = token_regex or r"\d+"
                fname = os.path.basename(path_in)
                base, ext = os.path.splitext(fname)
                m = re.match(rf"^(.*?){re.escape(sep)}({pat})$", base)
                if not m:
                    return path_in
                new_base = m.group(1)
                new_name = new_base + ext
                candidate = os.path.join(os.path.dirname(path_in), new_name)
                if os.path.abspath(candidate) == os.path.abspath(path_in):
                    return path_in
                if os.path.exists(candidate):
                    b2, e2 = os.path.splitext(candidate)
                    n = 1
                    while os.path.exists(f"{b2}_{n}{e2}"):
                        n += 1
                    candidate = f"{b2}_{n}{e2}"
                try:
                    os.rename(path_in, candidate)
                    log_fn(f"🧹 Renamed (strip token): {os.path.basename(path_in)} -> {os.path.basename(candidate)}")
                    return candidate
                except Exception as e:
                    log_fn(f"⚠️ Could not strip token for {os.path.basename(path_in)}: {e}")
                    return path_in
            ...
            try:
                ...
                # After placing, optionally strip token
                dest_path = _maybe_strip_token(dest_path)
...
class AllInOneApp(tk.Tk):
    ...
    def _build_tab_guide(self):
        ...
        title = ttk.Label(self.tab_guide, text="How to use this tool", font=("Segoe UI", 14, "bold"))
        title.pack(anchor="w", **pad)

        note = ttk.Label(
            self.tab_guide,
            text=(
                "This tool helps you rebuild the original folder structure after running OCR/PDF Tools that outputs a flat folder.\n"
                "Typical flow: 1) Append a unique token, 2) OCR, 3) Remove _OCR, 4) Rebuild folders, 5) Strip token."
            ),
            justify="left",
        )
        note.pack(anchor="w", padx=10)

        txt = tk.Text(self.tab_guide, wrap="word", height=34)
        txt.pack(fill="both", expand=True, padx=10, pady=10)

        guide = """
STEP-BY-STEP (recommended workflow)

1) Pre-OCR: Append Token  (Tab: Pre-OCR: Append Token)
   Goal: make every file name unique BEFORE OCR, so the OCR tool never creates _001 duplicates.
   - Mode: Copy to staging (recommended)
   - Token: Global counter (walk order)
   - Counter padding: 6 (gives 000001, 000002, ...)
   - Separator: __  (double underscore)
   Output (in staging):  originalname__000001.ext

   What you do next:
   - In PDF Tools, pick the STAGING folder as input.

2) Run PDF Tools / OCR on the STAGING folder
   - PDF Tools usually outputs a flat folder of PDFs, and appends a suffix like _OCR
   Example output:
     originalname__000001_OCR.pdf

3) Post-OCR: Remove _OCR  (Tab: Post-OCR: Remove _OCR)
   Goal: remove the _OCR part so the rebuild step can find files by name reliably.
   - Flat OCR output folder: the PDF Tools OUTPUT folder
   - Output ext: .pdf
   - Suffix to remove: _OCR
   Output becomes:
     originalname__000001.pdf

4) Rebuild folder structure (choose ONE of these)
   A) Rebuild: Original Folder Structure (template)  (Tab: Rebuild: Original Folder Structure)
      Use this if you still have the original nested folder structure available.
      - Original root: your ORIGINAL source structure (not staging)
      - Flat output folder: the folder from step 3 (now without _OCR)
      - Destination root: where you want the recreated structure to go
      - Output ext: .pdf
      - Output suffix: (leave blank if you already removed _OCR)
      - Strip token after placing: ON (recommended)
        This removes __000001 so final files become clean names again.

   B) Rebuild: XLSX Mapping  (Tab: Rebuild: XLSX Mapping)
      Use this if you have a spreadsheet that contains the original Directory/Name information.
      - Choose the spreadsheet + worksheet tab
      - Pick the Directory column (full path including filename) and Name column
      - Flat output folder: the folder from step 3
      - Destination root: where you want the recreated structure to go
      - Strip token after placing: ON (recommended)

REPORTS / OUTPUT WORKBOOKS

- Pre-OCR tab can write a report workbook showing original path -> new staged path.
- Post-OCR tab can write a report workbook showing old OCR names -> new names without suffix.
- Rebuild tabs can write ONE workbook with sheets:
  Summary, Missing, Duplicates, Orphans

COMMON PITFALLS

- If Destination root == Flat output folder AND Action is Move, the tool will block it (to prevent chaos).
- If PDF Tools still creates _001 duplicates even after tokens, it usually means the OCR tool changed names.
  In that case, keep the token AND capture the exact output naming pattern, then we can adapt matching.

QUICK TEST

Before doing 100k files, test on one small folder:
- Run token staging on ~20 files
- OCR them
- Remove _OCR
- Rebuild into a destination folder
- Confirm filenames are clean (token stripped)
...
# ---------------- Tab 0 ----------------
    ...
    # ---------------- Tab Post-OCR ----------------
    def _build_tab_postocr(self):
        pad = {"padx": 10, "pady": 6}

        self.o_flat = tk.StringVar()
        self.o_report = tk.StringVar()
        self.o_output_ext = tk.StringVar(value=".pdf")
        self.o_suffix = tk.StringVar(value="_OCR")

        frm = ttk.Frame(self.tab_postocr)
        frm.pack(fill="x", **pad)
        frm.columnconfigure(0, weight=1)

        ttk.Label(frm, text="Flat OCR output folder (from PDF Tools):").grid(row=0, column=0, sticky="w")
        ttk.Entry(frm, textvariable=self.o_flat, width=120).grid(row=1, column=0, sticky="we")
        ttk.Button(frm, text="Browse…", command=lambda: self._pick_folder(self.o_flat)).grid(row=1, column=1, padx=8)

        ttk.Label(frm, text="Output ext:").grid(row=2, column=0, sticky="w")
        ttk.Entry(frm, textvariable=self.o_output_ext, width=10).grid(row=3, column=0, sticky="w")

        ttk.Label(frm, text="Suffix to remove (e.g. _OCR):").grid(row=4, column=0, sticky="w")
        ttk.Entry(frm, textvariable=self.o_suffix, width=10).grid(row=5, column=0, sticky="w")

        ttk.Label(frm, text="Report workbook (.xlsx):").grid(row=6, column=0, sticky="w")
        ttk.Entry(frm, textvariable=self.o_report, width=120).grid(row=7, column=0, sticky="we")
        ttk.Button(frm, text="Save as…", command=lambda: self._pick_report(self.o_report)).grid(row=7, column=1, padx=8)

        btns = ttk.Frame(self.tab_postocr)
        btns.pack(fill="x", **pad)
        self.o_run_btn = ttk.Button(btns, text="Run", command=self._run_postocr)
        self.o_run_btn.pack(side="left")
        ttk.Button(btns, text="Clear Log", command=lambda: self._clear_log(self.o_log)).pack(side="left", padx=10)

        self.o_log = tk.Text(self.tab_postocr, height=22, wrap="word")
        self.o_log.pack(fill="both", expand=True, padx=10, pady=10)
        self._log(self.o_log, "This tab renames flat outputs like base__000001_OCR.pdf -> base__000001.pdf")

    def _run_postocr(self):
        flat = self.o_flat.get().strip()
        report = self.o_report.get().strip()
        output_ext = self.o_output_ext.get().strip() or ".pdf"
        suffix = self.o_suffix.get().strip() or "_OCR"

        if not flat:
            messagebox.showerror("Missing info", "Please select the flat OCR output folder.")
            return

        self.o_run_btn.config(state="disabled")
        self._log(self.o_log, "")
        self._log(self.o_log, "Running…")

        def worker():
            try:
                strip_suffix_in_flat_outputs(
                    flat_folder=flat,
                    output_ext=output_ext,
                    suffix_to_remove=suffix,
                    report_path=report,
                    log_fn=lambda m: self.after(0, self._log, self.o_log, m),
                )
                self.after(0, lambda: messagebox.showinfo("Done", "Finished removing suffix from flat outputs."))
            except Exception as e:
                self.after(0, lambda: messagebox.showerror("Error", str(e)))
            finally:
                self.after(0, lambda: self.o_run_btn.config(state="normal"))
...
# ---------------- Tab 1 ----------------
    def _build_tab_template(self):
        ...
        self.t_strip_token = tk.BooleanVar(value=True)
        self.t_token_sep = tk.StringVar(value="__")
        self.t_token_regex = tk.StringVar(value=r"\d+")

        ttk.Checkbutton(opts, text="Strip token after placing", variable=self.t_strip_token).pack(side="left", padx=(18, 0))
        ttk.Label(opts, text="Token sep:").pack(side="left", padx=(12, 0))
        ttk.Entry(opts, textvariable=self.t_token_sep, width=6).pack(side="left", padx=6)
        ttk.Label(opts, text="Token regex:").pack(side="left", padx=(12, 0))
        ttk.Entry(opts, textvariable=self.t_token_regex, width=10).pack(side="left", padx=6)
        ...
        def worker():
            try:
                recreate_from_original_tree(
                    ...
                    output_ext=output_ext,
                    output_suffix=output_suffix,
                    strip_token_after_place=bool(self.t_strip_token.get()),
                    token_separator=self.t_token_sep.get(),
                    token_regex=self.t_token_regex.get(),
                    report_path=report,
    ...
    def _build_tab_xlsx(self):
        ...
        self.x_strip_token = tk.BooleanVar(value=True)
        self.x_token_sep = tk.StringVar(value="__")
        self.x_token_regex = tk.StringVar(value=r"\d+")

        ttk.Checkbutton(inner, text="Strip token after placing", variable=self.x_strip_token).grid(row=4, column=0, sticky="w", pady=(8, 0))
        tfrm = ttk.Frame(inner)
        tfrm.grid(row=4, column=1, sticky="w", padx=10, pady=(8, 0))
        ttk.Label(tfrm, text="Sep").pack(side="left")
        ttk.Entry(tfrm, textvariable=self.x_token_sep, width=6).pack(side="left", padx=6)
        ttk.Label(tfrm, text="Regex").pack(side="left", padx=(10, 0))
        ttk.Entry(tfrm, textvariable=self.x_token_regex, width=10).pack(side="left", padx=6)
        ...
        def worker():
            try:
                process_from_excel_mapping(
                    ...
                    output_ext=output_ext,
                    output_suffix=output_suffix,
                    strip_token_after_place=bool(self.x_strip_token.get()),
                    token_separator=self.x_token_sep.get(),
                    token_regex=self.x_token_regex.get(),
